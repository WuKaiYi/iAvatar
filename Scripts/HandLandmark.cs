...
namespace LandmarkInterface
{
    public class HandLandmark : MonoBehaviour
    {
      ...
      //  public IRigLayer[] m_RuntimeRigLayers;
        ...
        public void updateLandmarkPosition(List<Vector3> landmarks)
        {
            ...
            for (int i = 1; i < landmarks.Count; i++)
            {
              ...
              //  LandmarkObjects[i].transform.position = new Vector3(landmarks[i].x, landmarks[i].y, landmarks[i].z);
            }
            ...
            //this.transform.position = handmark.position;

  
        }

        //correct landmark scale based on thumb length
        public void updateLandmarkScale(List<Vector3> list)
        {
        ...
        public void updateWristRotation()
        {
           var wristTransform = LandmarkObjects[0].transform;
            //var wristTransform =handmark;
            var indexFinger = LandmarkObjects[5].transform.position;
            var middleFinger = LandmarkObjects[17].transform.position;
          ...
          //  handmark.rotation = Quaternion.LookRotation(vectorToIndex, normalVector);
         //   handmark.localEulerAngles+= offr;
          //  handmark.localScale = offs;
        }
